...
/**
 * Aplica la configuracion de fisica calibrada para comportamiento
 * equivalente a Blender. Llama setPhysicsMaxSubSteps y setPhysicsFixedTimeStep
 * que existen en el runtime WASM pero no estan declarados en los tipos
 * TypeScript de babylon-mmd@1.0.0. El cast a any y la verificacion
 * typeof garantizan que el codigo es seguro en cualquier version.
 *
 * maxSubSteps=10 + fixedTimeStep=1/120 reproduce la configuracion
 * de 120Hz de simulacion interna que usa Blender por defecto, eliminando
 * la vibracion visible en articulaciones de cabello y ropa finos.
 */
function applyBlenderPhysicsConfig(runtime: MmdRuntime): void {
    const r = runtime as any;
    if (typeof r.setPhysicsMaxSubSteps === "function") {
        r.setPhysicsMaxSubSteps(10);
    }
    if (typeof r.setPhysicsFixedTimeStep === "function") {
        r.setPhysicsFixedTimeStep(1 / 120);
    }
}

/**
 * Warmup de fisica: equivalente exacto a MMDPhysics.warmup(frames) de three-stdlib.
 *
 * Ejecuta N pasos de simulacion sincrona antes del primer frame visible.
 * Esto asienta cabello, ropa y accesorios en su posicion de equilibrio correcta,
 * eliminando el salto visual donde los cuerpos rigidos caen desde su bind pose.
 *
 * El patron beforePhysics(dt) + afterPhysics() reproduce el ciclo completo de
 * actualizacion de fisica del runtime sin avanzar el tiempo de animacion,
 * de forma equivalente a lo que hace el solver de Bullet en Blender
 * durante el baking inicial de simulacion.
 *
 * Usar 60 frames como minimo. Para modelos con mucha ropa o cabello muy largo,
 * aumentar a 120.
 */
function runPhysicsWarmup(runtime: MmdRuntime, frames: number): void {
    const r = runtime as any;
    const dt = 1 / 60;
    for (let i = 0; i < frames; i++) {
        if (typeof r.beforePhysics === "function") {
            r.beforePhysics(dt, undefined);
        }
        if (typeof r.afterPhysics === "function") {
            r.afterPhysics();
        }
    }
}
...
/**
 * Reset robusto de cuerpos rigidos tras un seek de tiempo.
 *
 * El problema: cuando el usuario salta en la linea de tiempo, los cuerpos
 * rigidos mantienen velocidades lineales y angulares del frame anterior.
 * Al reanudar desde una posicion diferente, esas velocidades producen
 * una explosion visual instantanea (cuerpos volando en todas direcciones).
 *
 * Equivale al reset() de MMDPhysics de three-stdlib, que llama reset()
 * en cada rigidBody individualmente. En babylon-mmd usamos
 * initializeAllMmdModelsPhysics(true) que realiza la misma operacion
 * a traves del runtime WASM. El warmup posterior re-asienta los cuerpos
 * en la nueva posicion antes del primer frame visible.
 */
function resetPhysicsAfterSeek(runtime: MmdRuntime): void {
    if (typeof runtime.initializeAllMmdModelsPhysics === "function") {
        runtime.initializeAllMmdModelsPhysics(true);
    }
    runPhysicsWarmup(runtime, 30);
}
...
export class SceneBuilder implements ISceneBuilder {
    ...
    RegisterDxBmpTextureLoader();
		...
		const PHYSICS_GRAVITY = -30; // Cambia este valor para ajustar la gravedad global

        /**
         * Congela o descongela la simulacion de fisica poniendo la gravedad
         * a cero cuando la animacion se pausa.
         *
         * Por que es necesario:
         * Bullet Physics sigue ejecutando pasos de simulacion entre frames
         * aunque la animacion MMD este en pausa. Los cuerpos rigidos que
         * representan cabello y ropa continuan recibiendo impulsos gravitatorios
         * acumulando error numerico que se manifiesta como vibracion visible.
         * Al poner la gravedad a cero en pausa, los cuerpos rigidos mantienen
         * su posicion y velocidad actuales sin acumular error adicional.
         * Al reanudar, la gravedad vuelve al valor original y la simulacion
         * continua desde el estado correcto.
         */
        function setPhysicsPaused(paused: boolean): void {
            if (!physicsRuntime) return;
            physicsRuntime.setGravity(new Vector3(0, paused ? 0 : PHYSICS_GRAVITY, 0));
        }
        ...
        if (physicsModeOn) {
            ...
            /**
             * Configuracion de fisica calibrada para comportamiento similar a Blender.
             *
             * El problema raiz por el que la fisica MMD no se parece a Blender:
             * Bullet Physics (el motor subyacente) simula en pasos de tiempo fijos.
             * Si el paso es demasiado grande, las articulaciones "explotan" o el cabello
             * se comporta de forma errante. Blender usa por defecto 60 sub-pasos por segundo
             * con iteraciones de solver elevadas, lo que da movimientos suaves y estables.
             *
             * Parametros clave:
             * - setPhysicsMaxSubSteps(10): divide cada frame de render en hasta 10 pasos
             *   internos de simulacion. A 30fps = pasos de ~3.3ms. Mas estable.
             * - setPhysicsFixedTimeStep(1/120): cada paso simula 1/120 de segundo (~8.3ms).
             *   Equivale a 120Hz de simulacion, similar a la configuracion alta de Blender.
             * - gravedad -98: unidad MMD es 1cm = 1 unidad. La gravedad real en cm/s^2 es 980,
             *   pero el escalado del modelo (worldScale = 0.09) hace que -98 se comporte
             *   visualmente como gravedad real. Ajustar entre -50 (flotante) y -98 (pesado).
             */
            physicsRuntime.setGravity(new Vector3(0, PHYSICS_GRAVITY, 0));
            physicsRuntime.register(scene);

            const mmdBulletPhysics = new MmdBulletPhysics(physicsRuntime);
            mmdRuntime = new MmdRuntime(scene, mmdBulletPhysics);
            mmdRuntime.loggingEnabled = false;
            mmdRuntime.autoPhysicsInitialization = false;
            mmdRuntime.register(scene);
            applyBlenderPhysicsConfig(mmdRuntime);
        ...
        /**
         * Carga una animacion de movimiento de forma inteligente.
         * Detecta la extension del archivo y elige el loader correcto:
         * - .vmd  -> VmdLoader  (formato original MikuMikuDance)
         * - .bvmd -> BvmdLoader (formato binario optimizado de babylon-mmd)
         * Si la carga falla, devuelve undefined en lugar de lanzar una excepcion,
         * permitiendo que la escena continue sin la animacion.
         */
        const safeLoadBvmd = async (type: string, url: string | undefined | null, onProgress?: any) => {
            if (!url) {
                console.warn("safeLoadBvmd: url is undefined or empty, skipping.");
                return undefined;
            }
            const ext = url.split("?")[0].split(".").pop()?.toLowerCase();
            try {
                if (ext === "vmd") {
                    return await vmdLoader.loadAsync(type, url, onProgress);
                }
                return await bvmdLoader.loadAsync(type, url, onProgress);
            } catch (e: any) {
                console.warn(`Motion load failed (${ext ?? "unknown"}), continuing without motion:`, e?.message || e);
                return undefined;
            }
        };
        ...
        if (physicsModeOn) {
            scene.onAfterRenderObservable.addOnce(() => {
                ...
                // Warmup inicial: asienta la simulacion antes del primer frame visible.
                // Sin esto los cuerpos rigidos caen desde su posicion de bind pose
                // produciendo un salto visual en el primer segundo de carga.
                runPhysicsWarmup(mmdRuntime, 60);
            });
        }
        ...
        async function changeMotion(): Promise<void> {
            ...
            // Reset de cuerpos rigidos tras el seek para eliminar velocidades residuales.
            // Sin esto los cuerpos mantienen inercia del frame anterior y explotan
            // al reanudar desde frame 0 con una posicion de huesos completamente diferente.
            if (physicsModeOn) {
                resetPhysicsAfterSeek(mmdRuntime);
            }
        ...
        function resumePlayback(): void {
            if (previousModelState.wasAnimationPlaying) {
                ...
                // Reset de fisica tras seek al frame donde el usuario estaba.
                // El nuevo personaje tiene una posicion de bind pose diferente,
                // por lo que cualquier velocidad residual produce una explosion visual.
                if (physicsModeOn) {
                    resetPhysicsAfterSeek(mmdRuntime);
                }
        ...
        async function createCharacter(chosenChar?: BaseCharData|undefined): Promise<void> {
            ...
            if (physicsModeOn) {
                scene.onAfterRenderObservable.addOnce(() => {
                    mmdRuntime.initializeAllMmdModelsPhysics(true);
                    // Warmup cada vez que el usuario cambia de personaje.
                    // Equivalente al warmup(60) de three-stdlib ejecutado
                    // al montar el componente Physics en React.
                    runPhysicsWarmup(mmdRuntime, 60);
                });
            }
        ...
        /**
         * Sincronizacion de fisica con el estado de reproduccion y con el seek
         * del slider de la linea de tiempo.
         *
         * Dos responsabilidades:
         *
         * 1. Pausa/reproduccion: el player control llama pauseAnimation/playAnimation
         *    internamente sin pasar por el spacebar handler. El observer detecta el
         *    cambio y sincroniza la gravedad.
         *
         * 2. Seek del slider: cuando el usuario arrastra la barra de tiempo,
         *    currentFrameTime salta mas de 3 frames en un solo tick. Eso indica
         *    un seek manual. En ese caso se resetean los cuerpos rigidos para
         *    evitar velocidades residuales que producen la explosion visual.
         *    El umbral de 3 frames cubre variaciones normales de delta sin
         *    generar falsos positivos durante la reproduccion a velocidad normal.
         *
         * Costo total: dos comparaciones por frame, negligible.
         */
        let physicsSyncLastPlayState = mmdRuntime.isAnimationPlaying;
        let physicsSyncLastFrame = mmdRuntime.currentFrameTime;
        scene.onBeforeRenderObservable.add(() => {
            const isPlaying = mmdRuntime.isAnimationPlaying;
            if (isPlaying !== physicsSyncLastPlayState) {
                physicsSyncLastPlayState = isPlaying;
                setPhysicsPaused(!isPlaying);
            }

            // Detectar seek manual del slider de la linea de tiempo.
            // Un salto de mas de 3 frames en un tick indica seek, no reproduccion normal.
            const currentFrame = mmdRuntime.currentFrameTime;
            const frameDelta = Math.abs(currentFrame - physicsSyncLastFrame);
            if (frameDelta > 3 && physicsModeOn) {
                resetPhysicsAfterSeek(mmdRuntime);
            }
            physicsSyncLastFrame = currentFrame;
        });
