...
export class SceneBuilder implements ISceneBuilder {
    ...
    RegisterDxBmpTextureLoader();
		...
		// ── Boton Up Blender ───────────────────────────────────────────────────────
		const upBlenderButton = gui.Button.CreateSimpleButton("upBlenderBtn", "Up Blender");
		upBlenderButton.width         = isMobile ? "140px" : "100px";
		upBlenderButton.height        = isMobile ? "50px"  : "50px";
		upBlenderButton.color         = "white";
		upBlenderButton.background    = "rgba(200, 100, 20, 0.88)";
		upBlenderButton.cornerRadius  = 5;
		upBlenderButton.thickness     = 0;
		upBlenderButton.fontSize      = isMobile ? 16 : 11;
		upBlenderButton.fontWeight    = "bold";
		upBlenderButton.horizontalAlignment = gui.Control.HORIZONTAL_ALIGNMENT_RIGHT;
		upBlenderButton.verticalAlignment   = gui.Control.VERTICAL_ALIGNMENT_TOP;
		upBlenderButton.left  = "-10px";
		upBlenderButton.top   = isMobile ? "390px" : "600px";
		advancedTexture.addControl(upBlenderButton);

		upBlenderButton.onPointerClickObservable.add(() => {
			// Recopilar todos los parametros actuales de la escena
			const blenderPackage = {
				version: "1.0",
				...
				model: {
					...
					directory: chosenChar?.directory ?? "",
					pmx:       chosenChar?.pmx       ?? "",
					element:   chosenChar?.element   ?? "",
					// Nombre limpio del archivo PMX para buscarlo en la carpeta local
					pmxFileName: (chosenChar?.pmx ?? "").split("/").pop() ?? "",
				},

				// Animacion y musica
				motion: {
					name:            motionName         ?? "",
					modelMotionFile: modelMotionFile    ?? "",
					camMotionFile:   camMotionFile      ?? "",
					audioFile:       audioPlayerFile    ?? "",
				},

				// Estado de reproduccion
				playback: {
					currentFrame:  mmdRuntime.currentFrameTime,
					isPlaying:     mmdRuntime.isAnimationPlaying,
					volume:        audioPlayer?.volume ?? 1.0,
				},

				// Camara MMD (posicion y rotacion al frame actual)
				camera: {
					type:     "MmdCamera",
					position: {
						x: mmdCamera.position.x,
						y: mmdCamera.position.y,
						z: mmdCamera.position.z,
					},
					rotation: {
						x: mmdCamera.rotation.x,
						y: mmdCamera.rotation.y,
						z: mmdCamera.rotation.z,
					},
					distance: mmdCamera.distance,
					fov:      mmdCamera.fov,
				},

				// Post-proceso / Color Grading
				colorGrading: {
					...
					aberrationAmount:    defaultPipeline.chromaticAberration.aberrationAmount,
					highlightsSaturation:colorCurves.highlightsSaturation,
					midtonesSaturation:  colorCurves.midtonesSaturation,
					shadowsSaturation:   colorCurves.shadowsSaturation,
					highlightsDensity:   colorCurves.highlightsDensity,
					midtonesDensity:     colorCurves.midtonesDensity,
				...
				lighting: {
					...
					background: bg_bool ? "dark" : "light",
				},

				// Instrucciones para el script de Blender
				_instructions: [
					"1. Abre Blender.",
					"2. Ve a Scripting > Abrir script > selecciona 'blender_mmd_import.py'.",
					"3. Ajusta la variable ROOT_PATH al directorio raiz de tus assets.",
					"4. Corre el script (Run Script). Importara el modelo PMX y configurara la escena.",
				]
			};

			const charSlug = (chosenChar?.name ?? "scene").replace(/\s+/g, "_");

			// ── Funcion auxiliar para disparar una descarga desde una URL ─────────
			const downloadFromUrl = async (url: string, filename: string): Promise<void> => {
				try {
					const response = await fetch(url);
					if (!response.ok) {
						console.warn(`[Up Blender] No se pudo descargar: ${url} (${response.status})`);
						return;
					}
					...
					console.log(`[Up Blender] Descargado: ${filename}`);
				} catch (e) {
					console.warn(`[Up Blender] Error descargando ${filename}:`, e);
				}
			};

			// ── 1. Descargar JSON principal ───────────────────────────────────────
			const jsonStr    = JSON.stringify(blenderPackage, null, 2);
			const jsonBlob   = new Blob([jsonStr], { type: "application/json" });
			const jsonObjUrl = URL.createObjectURL(jsonBlob);
			const jsonAnchor = document.createElement("a");
			jsonAnchor.href     = jsonObjUrl;
			jsonAnchor.download = `blender_scene_${charSlug}.json`;
			document.body.appendChild(jsonAnchor);
			jsonAnchor.click();
			document.body.removeChild(jsonAnchor);
			URL.revokeObjectURL(jsonObjUrl);
			console.log(`[Up Blender] JSON descargado: blender_scene_${charSlug}.json`);

			// ── 2. Descargar modelo usando la misma logica del boton PMX ──────────
			// El boton PMX descarga correctamente con texturas empaquetadas.
			// Disparamos el mismo flujo pero con un nombre de archivo identificable.
			if (chosenChar && chosenChar.directory && chosenChar.pmx) {
				setTimeout(() => {
					// Reusar exactamente el mismo mecanismo del boton PMX existente
					const pmxCtrl = advancedTexture.getControlByName("downloadBtn") as gui.Button | null;
					if (pmxCtrl) {
						// Simular un clic en el boton PMX para que descargue el modelo
						pmxCtrl.onPointerClickObservable.notifyObservers(
							{} as any, -1, pmxCtrl, pmxCtrl
						);
						console.log("[Up Blender] Modelo: disparado via boton PMX");
					} else {
						console.warn("[Up Blender] No se encontro el boton PMX");
					}
				}, 500);
			}

			// ── 3. Exportar motions como VMD desde la animacion ya cargada en memoria
        // Capturar todas las variables ANTES del setTimeout para evitar que cambien
        // por referencia antes de que se ejecute el callback.
        {
            const capturedAnimation    = theCharAnimation;
            const capturedCamAnimation = loadResults[0] as any;
            const capturedCharName     = chosenCharName || "model";
            const capturedCharSlug     = charSlug;
            const capturedModelFile    = modelMotionFile ?? "";
            const capturedCamFile      = camMotionFile ?? "";

            setTimeout(() => {
                // Motion del modelo
                if (capturedAnimation) {
                    const vmdBlob = convertMmdAnimationToVmdBlob(
                        capturedAnimation,
                        false,
                        capturedCharName
                    );
                    if (vmdBlob) {
                        const motionOrigName = capturedModelFile.split("/").pop()?.replace(".bvmd", "") ?? "motion";
                        downloadBlob(vmdBlob, `motion_model_${capturedCharSlug}_${motionOrigName}.vmd`);
                        console.log("[Up Blender] VMD modelo descargado");
                    } else {
                        console.warn("[Up Blender] No se pudo convertir motion del modelo");
                    }
                } else {
                    console.warn("[Up Blender] No hay animacion de modelo cargada en memoria");
                }

                // Motion de la camara
                if (capturedCamAnimation) {
                    const vmdCamBlob = convertMmdAnimationToVmdBlob(
                        capturedCamAnimation,
                        true,
                        "Camera"
                    );
                    if (vmdCamBlob) {
                        const camOrigName = capturedCamFile.split("/").pop()?.replace(".bvmd", "") ?? "camera";
                        downloadBlob(vmdCamBlob, `motion_camera_${capturedCharSlug}_${camOrigName}.vmd`);
                        console.log("[Up Blender] VMD camara descargado");
                    } else {
                        console.warn("[Up Blender] No se pudo convertir motion de camara");
                    }
                } else {
                    console.warn("[Up Blender] No hay animacion de camara cargada en memoria");
                }
            }, 2000);
        }

			console.log("[Up Blender] Paquete completo iniciado. Archivos esperados:", [
				`blender_scene_${charSlug}.json`,
				`${charSlug}_model.zip (via boton PMX)`,
				`motion_model_${charSlug}_....vmd`,
				`motion_camera_${charSlug}_....vmd`
			]);
		});
		...
		// ── Conversor BVMD -> VMD ─────────────────────────────────────────────
        // Toma un MmdAnimation ya cargado en memoria y genera un Blob en formato
        // VMD binario nativo que mmd_tools de Blender puede leer.
        //
        // Especificacion VMD:
        //   Header:  30 bytes firma + 20 bytes nombre del modelo
        //   Huesos:  uint32 count, luego cada registro de 111 bytes
        //   Morphs:  uint32 count, luego cada registro de 23 bytes
        //   Camara:  uint32 count, luego cada registro de 61 bytes
        //
        // Los datos de rotacion en VMD son quaterniones en formato xyzw little-endian.
        // Los datos de posicion en VMD estan en unidades MikuMikuDance (centimetros).
        // babylon-mmd almacena en metros con escala worldScale, por lo que se
        // multiplica por 12.5 (factor empirico para coincidir con MMD original).

        function encodeSjisFixed(text: string, byteLength: number): Uint8Array {
            // Codifica un string como ASCII/latin truncado o con padding de ceros.
            // VMD usa Shift-JIS pero los nombres de huesos MMD estandar son ASCII
            // o japones. Para compatibilidad maxima usamos UTF-8 truncado con null.
            const result = new Uint8Array(byteLength);
            for (let i = 0; i < Math.min(text.length, byteLength - 1); i++) {
                result[i] = text.charCodeAt(i) & 0xff;
            }
            return result;
        }

        function convertMmdAnimationToVmdBlob(
            animation: any,
            isCamera: boolean,
            modelName: string
        ): Blob | null {
            try {
                // ── Calcular tamano total del buffer ──────────────────────────
                // Header fijo: 30 (firma) + 20 (nombre modelo) = 50 bytes
                const HEADER_SIZE = 50;

                // Recopilar datos de huesos y morphs del MmdAnimation
                const boneTrackNames: string[]   = [];
                const boneFrames: any[][]         = [];
                const morphTrackNames: string[]  = [];
                const morphFrames: any[][]        = [];

                let totalBoneRecords  = 0;
                let totalMorphRecords = 0;
                let totalCameraRecords = 0;

                if (!isCamera) {
                    // Pistas de huesos movibles (con posicion y rotacion)
                    const movableBoneCount: number = animation.movableBoneTrackCount ?? 0;
                    for (let i = 0; i < movableBoneCount; i++) {
                        const name: string   = animation.getMovableBoneTrackName?.(i) ?? `bone_m${i}`;
                        const frames: any[]  = animation.getMovableBoneTrackFrameNumbers?.(i) ?? [];
                        boneTrackNames.push(name);
                        boneFrames.push(frames);
                        totalBoneRecords += frames.length;
                    }
                    // Pistas de huesos solo-rotacion
                    const boneCount: number = animation.boneTrackCount ?? 0;
                    for (let i = 0; i < boneCount; i++) {
                        const name: string   = animation.getBoneTrackName?.(i) ?? `bone_${i}`;
                        const frames: any[]  = animation.getBoneTrackFrameNumbers?.(i) ?? [];
                        boneTrackNames.push(name);
                        boneFrames.push(frames);
                        totalBoneRecords += frames.length;
                    }
                    // Pistas de morphs
                    const morphCount: number = animation.morphTrackCount ?? 0;
                    for (let i = 0; i < morphCount; i++) {
                        const name: string   = animation.getMorphTrackName?.(i) ?? `morph_${i}`;
                        const frames: any[]  = animation.getMorphTrackFrameNumbers?.(i) ?? [];
                        morphTrackNames.push(name);
                        morphFrames.push(frames);
                        totalMorphRecords += frames.length;
                    }
                } else {
                    // Para camara, contamos los frames de la pista de camara
                    totalCameraRecords = animation.frameCount ?? 0;
                }

                // Tamano total:
                // Header: 50
                // Huesos: 4 + 111 * totalBoneRecords
                // Morphs: 4 + 23 * totalMorphRecords
                // Camara: 4 + 61 * totalCameraRecords
                // Luz:    4 + 0 registros
                // Sombra: 4 + 0 registros
                const bufferSize =
                    HEADER_SIZE +
                    4 + 111 * totalBoneRecords +
                    4 + 23  * totalMorphRecords +
                    4 + 61  * totalCameraRecords +
                    4 + // luz count = 0
                    4;  // sombra count = 0
                ...
                // ── Header ────────────────────────────────────────────────────
                // Firma VMD: "Vocaloid Motion Data 0002" + padding hasta 30 bytes
                const firma = "Vocaloid Motion Data 0002\0\0\0\0\0";
                for (let i = 0; i < 30; i++) {
                    bytes[offset + i] = firma.charCodeAt(i) & 0xff;
                }
                offset += 30;

                // Nombre del modelo (20 bytes, null-padded)
                const modelBytes = encodeSjisFixed(modelName, 20);
                bytes.set(modelBytes, offset);
                offset += 20;

                // ── Registros de huesos ───────────────────────────────────────
                // Cada registro: 15 bytes nombre + 4 frame + 12 pos + 16 rot + 64 interpolacion
                view.setUint32(offset, totalBoneRecords, true);
                offset += 4;

                if (!isCamera) {
                    const INTERPOL_DEFAULT = new Uint8Array(64).fill(20);
                    INTERPOL_DEFAULT[0]  = 20; INTERPOL_DEFAULT[4]  = 107;
                    INTERPOL_DEFAULT[8]  = 20; INTERPOL_DEFAULT[12] = 107;
                    INTERPOL_DEFAULT[16] = 20; INTERPOL_DEFAULT[20] = 107;
                    INTERPOL_DEFAULT[24] = 20; INTERPOL_DEFAULT[28] = 107;
                    INTERPOL_DEFAULT[32] = 20; INTERPOL_DEFAULT[36] = 107;
                    INTERPOL_DEFAULT[40] = 20; INTERPOL_DEFAULT[44] = 107;
                    INTERPOL_DEFAULT[48] = 20; INTERPOL_DEFAULT[52] = 107;
                    INTERPOL_DEFAULT[56] = 20; INTERPOL_DEFAULT[60] = 107;

                    for (let ti = 0; ti < boneTrackNames.length; ti++) {
                        const name      = boneTrackNames[ti];
                        const frameNums = boneFrames[ti];
                        const nameBytes = encodeSjisFixed(name, 15);

                        // Obtener arrays de datos para esta pista
                        const isMovable = ti < (animation.movableBoneTrackCount ?? 0);
                        let positions: Float32Array | null = null;
                        let rotations: Float32Array | null = null;

                        if (isMovable) {
                            const idx = ti;
                            positions = animation.getMovableBoneTrackPositions?.(idx) ?? null;
                            rotations = animation.getMovableBoneTrackRotations?.(idx) ?? null;
                        } else {
                            const idx = ti - (animation.movableBoneTrackCount ?? 0);
                            rotations = animation.getBoneTrackRotations?.(idx) ?? null;
                        }

                        for (let fi = 0; fi < frameNums.length; fi++) {
                            bytes.set(nameBytes, offset);
                            offset += 15;

                            view.setUint32(offset, frameNums[fi], true);
                            offset += 4;

                            // Posicion (x, y, z) en unidades MMD
                            if (positions && fi * 3 + 2 < positions.length) {
                                const scale = 12.5;
                                view.setFloat32(offset,      positions[fi * 3]     * scale, true);
                                view.setFloat32(offset + 4,  positions[fi * 3 + 1] * scale, true);
                                view.setFloat32(offset + 8, -positions[fi * 3 + 2] * scale, true);
                            }
                            offset += 12;

                            // Rotacion quaternion (x, y, z, w)
                            // babylon-mmd almacena como [x, y, z, w]
                            // VMD espera [x, y, z, w] pero con Z negado por diferencia de ejes
                            if (rotations && fi * 4 + 3 < rotations.length) {
                                view.setFloat32(offset,     -rotations[fi * 4],     true);
                                view.setFloat32(offset + 4, -rotations[fi * 4 + 1], true);
                                view.setFloat32(offset + 8,  rotations[fi * 4 + 2], true);
                                view.setFloat32(offset + 12, rotations[fi * 4 + 3], true);
                            } else {
                                // Quaternion identidad si no hay datos
                            ...
                            // Curvas de interpolacion (64 bytes, valores lineales por defecto)
                            bytes.set(INTERPOL_DEFAULT, offset);
                            offset += 64;
                        }
                    }
                }
                ...
                // ── Registros de morphs ───────────────────────────────────────
                // Cada registro: 15 bytes nombre + 4 frame + 4 peso (float)
                view.setUint32(offset, totalMorphRecords, true);
                offset += 4;

                if (!isCamera) {
                    for (let ti = 0; ti < morphTrackNames.length; ti++) {
                        const name      = morphTrackNames[ti];
                        const frameNums = morphFrames[ti];
                        const nameBytes = encodeSjisFixed(name, 15);
                        const weights: Float32Array | null = animation.getMorphTrackWeights?.(ti) ?? null;
                        ...
                        for (let fi = 0; fi < frameNums.length; fi++) {
                            ...
                            view.setUint32(offset, frameNums[fi], true);
                            offset += 4;
                            const w = weights ? weights[fi] : 0;
                            view.setFloat32(offset, w, true);
                            offset += 4;
                        }
                    }
                }
                ...
                // ── Registros de camara ───────────────────────────────────────
                // Cada registro: 4 frame + 4 distancia + 12 pos + 12 rot + 24 interpolacion + 4 fov + 1 perspectiva
                view.setUint32(offset, totalCameraRecords, true);
                offset += 4;

                if (isCamera && totalCameraRecords > 0) {
                    const positions: Float32Array | null = animation.cameraPositions  ?? null;
                    const rotations: Float32Array | null = animation.cameraRotations  ?? null;
                    const distances: Float32Array | null = animation.cameraDistances  ?? null;
                    const fovs:      Float32Array | null = animation.cameraFovs       ?? null;

                    const INTERPOL_CAM = new Uint8Array(24).fill(20);
                    INTERPOL_CAM[0]  = 20; INTERPOL_CAM[4]  = 107;
                    INTERPOL_CAM[8]  = 20; INTERPOL_CAM[12] = 107;
                    INTERPOL_CAM[16] = 20; INTERPOL_CAM[20] = 107;

                    for (let fi = 0; fi < totalCameraRecords; fi++) {
                        view.setUint32(offset, fi, true);
                        offset += 4;

                        const dist = distances ? distances[fi] : 45.0;
                        view.setFloat32(offset, -dist * 12.5, true);
                        offset += 4;
                        ...
                        if (rotations && fi * 3 + 2 < rotations.length) {
                            view.setFloat32(offset,      rotations[fi * 3],     true);
                            view.setFloat32(offset + 4,  rotations[fi * 3 + 1], true);
                            view.setFloat32(offset + 8, -rotations[fi * 3 + 2], true);
                        }
                        offset += 12;

                        bytes.set(INTERPOL_CAM, offset);
                        offset += 24;

                        const fovDeg = fovs ? Math.round(fovs[fi] * 180 / Math.PI) : 30;
                        view.setUint32(offset, fovDeg, true);
                        offset += 4;

                        bytes[offset] = 0; // perspectiva activada
                        offset += 1;
                    }
                }

                // ── Luz y sombra (cero registros) ─────────────────────────────
                ...
                console.error("[VMD Export] Error durante conversion:", err);
                return null;
            }
        }
